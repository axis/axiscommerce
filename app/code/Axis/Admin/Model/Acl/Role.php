...
class Axis_Admin_Model_Acl_Role extends Axis_Db_Table
{
    ...
    {
        return Axis::single('admin_acl_role_parent')->select('role_parent_id')
     ...
     * @param $parents array
     */
    public function saveParents($roleId, $parents)
    {
        Axis::single('admin/acl_role_parent')->delete(
            $this->getAdapter()->quoteInto('role_id = ?', $roleId)
        );

        $modelParent = Axis::single('admin/acl_role_parent');
        foreach ($parents as $parentId) {
            $modelParent->insert(array(
                'role_id'        => $roleId,
    ...
    public function saveRules($roleId, $rules)
    {
        /* remove old rules */
        $modelAclRule = Axis::single('admin/acl_rule');
        $modelAclRule->delete(
            $this->getAdapter()->quoteInto('role_id = ?', $roleId)
        );
        ...
        if (isset($rules['allow'])) {
            foreach ($rules['allow'] as $resourceId) {
                $modelAclRule->insert(array(
                    'role_id'     => $roleId,
        ...
        /* insert deny */
        if (isset($rules['deny'])) {
            foreach ($rules['deny'] as $resourceId) {
                $row = $modelAclRule->find($roleId, $resourceId)->current();
